...
const TodoList = () => {
  ...
  const handleCompletedItemClick = (postId, postTitle) => {
    if (window.confirm(`Are you sure you want to remove "${postTitle}" from the completed list?`)) {
      deleteTask(postId);
    }
  };
                        ...
                        {tabName === 'Completed' && post.completed && ( 
                          <div>
                            <p
                              ...
                              onClick={() => handleCompletedItemClick(post.id, post.title)} 
                            >
